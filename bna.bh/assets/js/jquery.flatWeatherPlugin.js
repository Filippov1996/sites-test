    ...
    $.extend(Plugin.prototype, {
        ...
        render: function(weather, context) {
            ...
            if (context.settings.view != "forecast") {
                ...
                $("<div/>", {
                    "class": "wi " + "wi" + weather.today.code
                }).appendTo(iconGroup);
                $("<p/>", {
                    "class": "wiText"
                }).text(weather.today.desc).appendTo(iconGroup);
                iconGroup.appendTo(today);
                $("<p/>", {
                    "class": "wiTemperature"
                }).html(weather.today.temp.now + "<sup>" + degrees + "</sup>").appendTo(today);
                today.appendTo(div);
            }
            ...
            if (context.settings.view != "simple") {
                ...
                if (context.settings.view == "partial") {
                    $("<p/>", {
                        "class": "wiDay"
                    }).text(weather.today.day).appendTo(today);
                }
                ...
                if (context.settings.view != "partial") {
                    if (context.settings.view != "today") {
                        $("<p/>", {
                            "class": "wiDay"
                        }).text(weather.today.day).appendTo(detail);
                    }
                    var astro = $("<ul/>", {
                        "class": "astronomy"
                    }).appendTo(detail);
                    $("<li/>", {
                        "class": "wi sunrise"
                    }).text(weather.today.sunrise).appendTo(astro);
                    $("<li/>", {
                        "class": "wi sunset"
                    }).text(weather.today.sunset).appendTo(astro);
                    var temp = $("<ul/>", {
                        "class": "temp"
                    }).appendTo(detail);
                    ...
                    $("<li/>", {
                        "class": "wi humidity"
                    }).text(weather.today.humidity).appendTo(atmo);
                    $("<li/>", {
                        "class": "wi pressure"
                    }).text(weather.today.pressure).appendTo(atmo);
                    $("<li/>", {
                        "class": "wi wind"
                    }).text(formatWind(weather.today.wind.speed, weather.today.wind.deg, context.settings.units)).appendTo(atmo);
                ...
                if (context.settings.view != "today" || context.settings.view == "forecast") {
                    ...
                    for (var i = startingIndex; i < weather.forecast.length; i++) {
                        var day = $("<li/>", {
                            "class": "wiDay"
                        }).html("<span>" + weather.forecast[i].day + "</span>").appendTo(forecast);
                        var sub = $("<ul/>", {
                            "class": "wiForecast"
                        }).appendTo(day);
                        $("<li/>", {
                            "class": "wi " + "wi" + weather.forecast[i].code
                        }).appendTo(sub);
                        $("<li/>", {
                            "class": "wiMax"
                        }).html(weather.forecast[i].temp.max + "<sup>" + degrees + "</sup>").appendTo(sub);
                        $("<li/>", {
                            "class": "wiMin"
                        }).html(weather.forecast[i].temp.min + "<sup>" + degrees + "</sup>").appendTo(sub);
                    }
    ...
    function datamapper(input, settings) {
        ...
        if (settings.api == "openweathermap") {
            ...
            for (var i = 0; i < settings.forecast; i++) {
                ...
                forecast.temp = {
                    max: Math.round(input[1].list[i].temp.max),
                    min: Math.round(input[1].list[i].temp.min)
                }
            ...
            for (var i = 0; i < settings.forecast; i++) {
                ...
                forecast.temp = {
                    max: Math.round(input.item.forecast[i].high),
                    min: Math.round(input.item.forecast[i].low)
                }
                out.forecast.push(forecast);
            }
        }
